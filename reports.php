...
if($start_date != '' && $end_date != '' && $report_type != '')
{
	...
	if($report_type == 'Bill')
	{
		...
		<div class="card mt-3">
			<div class="card-header">
				<div class="row">
					<div class="col-md-9">
						<h5 class="card-title">Bill Data for '.$start_date.' to '.$end_date.' Date</h5>
					</div>
					<div class="col-md-3">
						<a href="reports.php?action=export&type=Bill&start_date='.$start_date.'&end_date='.$end_date.'" class="btn btn-success btn-sm float-end">Export</a>
					</div>
				</div>
			</div>
			<div class="card-body">
		...
		$total_records = $pdo->query('SELECT COUNT(*) FROM bills WHERE created_at BETWEEN "'.$start_date.'" AND "'.$end_date.'"')->fetchColumn();
		...
		$stmt = $pdo->prepare('SELECT bills.bill_title, flats.block_number, flats.flat_number, bills.amount, bills.month, bills.paid_date, bills.payment_method, bills.paid_amount FROM bills INNER JOIN flats ON bills.flat_id = flats.id WHERE bills.created_at BETWEEN "'.$start_date.'" AND "'.$end_date.'" ORDER BY bills.id DESC LIMIT '.$offset.', '.$records_per_page.'');
		$stmt->execute();
		...
		if(count($records) > 0)
		{
			foreach($records as $record)
			{
				...
				$payment_method = ($record['payment_method'] != '') ? $record['payment_method'] : 'NA';
		...
		$output_data .= '
					...
					<nav aria-label="Page navigation">
					    <ul class="pagination justify-content-center">
							<li class="page-item' . ($current_page == 1 ? ' disabled' : '') . '"><a class="page-link" href=reports.php?type=Bill&start_date='.$start_date.'&end_date='.$end_date.'&page=' . ($current_page - 1) . '">Previous</a></li>
		';

		for ($i = 1; $i <= $total_pages; $i++) 
		{
			$output_data .= '
							<li class="page-item' . ($i == $current_page ? ' active' : '') . '"><a class="page-link" href="reports.php?type=Bill&start_date='.$start_date.'&end_date='.$end_date.'&page=' . $i . '">' . $i . '</a></li>';
		}

		$output_data .= '
							<li class="page-item' . ($current_page == $total_pages ? ' disabled' : '') . '"><a class="page-link" href="reports.php?type=Bill&start_date='.$start_date.'&end_date='.$end_date.'&page=' . ($current_page + 1) . '">Next</a></li>
						</ul>
					</nav>
				</div>
			</div>
	...
	if($report_type == 'Visitor')
	{
		...
		// Get the total number of records from the database
		$total_records = $pdo->query('SELECT COUNT(*) FROM visitors WHERE created_at BETWEEN "'.$start_date.'" AND "'.$end_date.'"')->fetchColumn();
			...
			foreach($records as $record)
			{
				...
				$output_data .= '
						<tr>
							...
							<td>'.$record["person_to_meet"].'</td>
							<td>'.$record["reason"].'</td>
							<td>'.$record["in_datetime"].'</td>
							<td>'.$record["out_remark"].'</td>
...
if(isset($_GET['action'], $_GET['type'], $_GET['start_date'], $_GET['end_date']))
{
	...
	if($action == 'export' && $report_type != '' && $start_date != '' && $end_date != '')
	{
		if($report_type == 'Bill')
		{
			$stmt = $pdo->prepare('SELECT bills.bill_title, flats.block_number, flats.flat_number, bills.amount, bills.month, bills.paid_date, bills.payment_method, bills.paid_amount FROM bills INNER JOIN flats ON bills.flat_id = flats.id WHERE bills.created_at BETWEEN "'.$start_date.'" AND "'.$end_date.'" ORDER BY bills.id DESC');
			$stmt->execute();
			$records = $stmt->fetchAll(PDO::FETCH_ASSOC);

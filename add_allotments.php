...
if(isset($_POST['add_allotment']))
{
    ...
    if (empty($move_in_date)) {
  	...
  	if (empty($errors)) 
  	{
  		$stmt = $pdo->prepare('INSERT INTO allotments (user_id, flat_id, flat_ownership, move_in_date, move_out_date, created_at) VALUES (?, ?, ?, ?, ?, ?)');
        
        $stmt->execute([$user_id, $flat_id, $flat_ownership, $move_in_date, $move_out_date, $created_at]);
		...
		<div class="card">
			...
			<div class="card-body">
				<form method="post">
  					...
  					<center><button type="submit" name="add_allotment" class="btn btn-primary" style="background-color:green">Add Allotment</button></center>
				</form>
			</div>
		</div>
	</div>
